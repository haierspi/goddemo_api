...
func (svc *Service) GoodsList(param *GoodsListRequest, pager *app.Pager) ([]*Goods, int, error) {
	...
	count, err := svc.dao.CountGoodsList(param.CategoryID)
	if err != nil {
		return nil, 0, err
	}
	daoList, err := svc.dao.GetGoodsList(param.CategoryID, pager.Page, pager.PageSize)
	if err != nil {
		return nil, 0, err
	}
...
func (svc *Service) GoodsDetails(param *GoodsDetailsRequest) (*Goods, error) {
	dao, err := svc.dao.GetGoodsById(param.GoodsID)
	if err != nil {
		return nil, err
	}
	return convert.StructAssign(dao, &Goods{}).(*Goods), nil
}

// GoodsDetailsURLKey 根据商品URL获取商品详情
func (svc *Service) GoodsDetailsURLKey(param *GoodsDetailsURLRequest) (*Goods, error) {
	dao, err := svc.dao.GetGoodsByURLKey(param.GoodsURL)
	if err != nil {
		return nil, err
	}
	return convert.StructAssign(dao, &Goods{}).(*Goods), nil
