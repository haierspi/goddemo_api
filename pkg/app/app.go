...
func (r *Response) SendResponse(statusCode int, content interface{}) {
	...
	_, exist = r.Ctx.Get("dis_encrypt")

	if exist == true {
		r.Ctx.JSON(http.StatusOK, content)
	} else if debug != "1" && global.SecuritySetting.EnableHtmlEncrypt {
		jsonCont, _ := json.Marshal(content)
		r.Ctx.String(http.StatusOK, Convert(string(jsonCont)))
